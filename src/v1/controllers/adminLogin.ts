...
// [POST] /adminLogin
const adminLoginHandler = async (req: ExpressRequest, res: Response): Promise<any> => {
  console.log("===== START : adminLoginHandler           =====")
  ...
  try {
    ...
    if (userRes.userRoleGlobal !== E_User_Role_Global.Admin)
      return responseFormatHttp(req, res, EResponseStatus.WARNING, { error: "user not role_global admin" })
    ...
    console.log("=====  END  : adminLoginHandler : Success =====")
    return responseFormatHttp(req, res, EResponseStatus.SUCCESS, { accessToken, refreshToken })
  } catch (error) {
    console.log("=====  END  : adminLoginHandler : Error   =====", error)
